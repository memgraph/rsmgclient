...
use rsmgclient::{Node,ConnectParams,Connection,QueryParam,str_to_c_str,ConnectionStatus,Value};
use maplit::hashmap;
use serial_test::serial;
use std::collections::HashMap;

struct Mockery {
    mg_session_params_make_ctx: rsmgclient::mock_params_make::__mg_session_params_make::Context,
    mg_connect_ctx: rsmgclient::mock_connect::__mg_connect::Context,
    mg_session_run_ctx: rsmgclient::mock_run::__mg_session_run::Context,
    mg_session_pull_ctx: rsmgclient::mock_pull::__mg_session_pull::Context,
    mg_session_error_ctx: rsmgclient::mock_mg_session_error::__mg_session_error::Context,
    mg_result_row_ctx: rsmgclient::mock_mg_result_row::__mg_result_row::Context,
}
    ...
    fn default() -> Self {
        Mockery {
            mg_session_params_make_ctx: {
                let ctx = rsmgclient::mock_params_make::mg_session_params_make_context();
                ctx.expect()
                    .returning(|| unsafe { rsmgclient::mg_session_params_make() });
                ctx
            },
            mg_connect_ctx: {
                let ctx = rsmgclient::mock_connect::mg_connect_context();
                ctx.expect()
                    .returning(|arg1, arg2| unsafe { rsmgclient::mg_connect(arg1, arg2) });
                ctx
            },
            mg_session_run_ctx: {
                let ctx = rsmgclient::mock_run::mg_session_run_context();
                ctx.expect().returning(|arg1, arg2, arg3, arg4| unsafe {
                    rsmgclient::mg_session_run(arg1, arg2, arg3, arg4)
                });
                ctx
            },
            mg_session_pull_ctx: {
                let ctx = rsmgclient::mock_pull::mg_session_pull_context();
                ctx.expect()
                    .returning(|arg1, arg2| unsafe { rsmgclient::mg_session_pull(arg1, arg2) });
                ctx
            },
            mg_session_error_ctx: {
                let ctx = rsmgclient::mock_mg_session_error::mg_session_error_context();
                ctx.expect()
                    .returning(|arg1| unsafe { rsmgclient::mg_session_error(arg1) });
                ctx
            },
            mg_result_row_ctx: {
                let ctx = rsmgclient::mock_mg_result_row::mg_result_row_context();
                ctx.expect()
                    .returning(|arg1| unsafe { rsmgclient::mg_result_row(arg1) });
                ctx
            },
        }
    }
...
fn error_while_pulling() {
    run_test(|mut mockery| {
        ...
        mockery.mg_session_pull_ctx = rsmgclient::mock_pull::mg_session_pull_context();
        mockery
            .mg_session_pull_ctx
            .expect()
            .returning(|_arg1, _arg2| 4);

        mockery.mg_result_row_ctx = rsmgclient::mock_mg_result_row::mg_result_row_context();
        mockery
            .mg_result_row_ctx
            .expect()
            .returning(|_arg1| std::ptr::null_mut());

        mockery.mg_session_error_ctx = rsmgclient::mock_mg_session_error::mg_session_error_context();
        mockery
            .mg_session_error_ctx
            .expect()
